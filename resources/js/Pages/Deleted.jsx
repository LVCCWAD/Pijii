...
export default function Deleted() {
  ...
  const handleDelete = (categoryId, projectId, projectName) => {
    if (confirm(`Are you sure you want to permanently delete "${projectName}"? This will delete all it's subprojects and tasks. This action cannot be undone.`)) {
      router.delete(
                  ...
                  <div className="flex flex-wrap gap-3">
                    {projects.map((project) => {
                      ...
                      return (
                        <div
                          key={project.id}
                          className="relative w-full md:w-72 bg-amber-50 rounded-lg shadow-sm transition-all duration-200 hover:bg-amber-100 hover:scale-[1.02] active:scale-95 active:bg-amber-200 p-3"
                        >
                          {/* Top right trash icon */}
                          <div className="absolute top-2 right-2">
                            <Tooltip label="Permanently delete project" position="bottom" withArrow>
                              <button
                                onClick={() => handleDelete(project.category_id, project.id, project.project_name || "Untitled")}
                          ...
                          <Link
                            ...
                            className="block font-medium text-sm no-underline text-black mb-1"
                          >
